<div class="inner-page-wrap">
	<div class="product-details--modal-page-100">
		<div class="d-flex justify-content-between align-items-center mb-3">
			<h3 class="page-title mb-1 f-15">
				<span>仕⼊れ情報を編集</span>
			</h3>
		</div>
		<!--div-->

		<form class="mb-4">
			<div class="form-group row">
				<label class="col-lg-3 col-form-label text-right pl-0">倉庫</label>
				<div class="col-lg-9">
					<select class="custom-select form-control">
						<option selected>横浜本社倉庫</option>
						<option value="1">横浜本社倉庫</option>
						<option value="2">横浜本社倉庫</option>
					</select>
				</div>
			</div>
			<div class="form-group row">
				<label class="col-lg-3 col-form-label text-right pl-0">デフォルト<br/>サプライヤー</label>
				<div class="col-lg-9">
					<select class="custom-select form-control">
						<option selected>Amazon</option>
						<option value="1">Shopify</option>
					</select>
				</div>
			</div>
			<div class="form-group row">
				<label class="col-lg-3 col-form-label text-right pl-0">参照URL</label>
				<div class="col-lg-9">
					<input type="text" class="form-control" placeholder="https://app.lismoa.co">
				</div>
			</div>
			<div class="form-group row url-clone">
				<label class="col-lg-3 col-form-label text-right pl-0">参照URL</label>
				<div class="col-lg-9">
					<input type="text" class="form-control" placeholder="https://app.lismoa.co">
				</div>
			</div>

			<p class="text-right add-more-url f-12"><a>さらにURLを追加する</a></p>

			<div class="form-check row d-flex">
				<label class="col-lg-3 col-form-label text-right pl-0">&nbsp;</label>
				<div class="col-lg-9 pl-4 f-12">
					<input class="form-check-input" type="checkbox" value="" id="defaultCheck1">
					<label class="form-check-label" for="defaultCheck1">
						他のバリエーションにもまとめて適⽤する
					</label>
				</div>
			</div>
		</form>

		<div class="d-flex justify-content-end">
			<a href="#" class="btn btn-red mr-1"><span>キャンセル</span></a>
			<a href="#" class="btn btn-dark"><span>保存する</span></a>
		</div>

	</div>
	<!--order-hold-modal-page w-100-->
</div>
<!--inner-page-wrap-->

