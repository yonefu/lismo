<div class="inner-page-wrap">
	<div class="product-details--modal-page w-100">
		<div class="d-flex justify-content-between align-items-center mb-3">
			<h3 class="page-title mb-1 f-15">
				<span>タグを編集</span>
			</h3>
		</div>
		<!--div-->

		<form class="mb-4">
			<div class="form-group row">
				<label class="col-lg-3 col-form-label text-right pl-0">商品タグ</label>
				<div class="col-lg-9">
					<div class="search-box">
						<div class="form-inline">
							<div class="input-group align-items-center">
								<input type="text" class="form-control" placeholder="iPhone">
								<button type="search" class="btn-search">検索</button>
								<span class="clearable"><img src="dist/img/icons/icon-close.png" alt="" /></span>
							</div>
						</div>
					</div>
					<!--search box-->
				</div>
			</div>
			<div class="form-group row">
				<label class="col-lg-3 col-form-label text-right pl-0">登録済タグ</label>
				<div class="col-lg-9">
					<div class="d-flex registerd-tags">
						<div class="tag btn btn-outline-dark br-25 mr-2 px-3">
							<span class="mr-2">iPhone</span>
							<i class="fa fa-close"></i>
						</div>
						<div class="tag btn btn-outline-dark br-25 mr-2 px-3">
							<span class="mr-2">Apple</span>
							<i class="fa fa-close"></i>
						</div>
					</div>
				</div>
			</div>
		</form>

		<div class="msg-text text-right mb-4 f-12">
			<p class="text-center">※タグはすべてのバリエーションで共通です</p>
		</div>

		<div class="d-flex justify-content-end">
			<a href="#" class="btn btn-red mr-1"><span>キャンセル</span></a>
			<a href="#" class="btn btn-dark"><span>保存する</span></a>
		</div>

	</div>
	<!--order-hold-modal-page w-100-->
</div>
<!--inner-page-wrap-->

